...
export default function QuotationSummaryCard({ request }) {
  ...
  const unitPrice = quantity > 0 ? Number(request.subTotal || 0) / quantity : 0;

  const calculatedSubTotal = editableItems.reduce(
  ...
  const sgst = Number(request.sgstAmount || 0);

  const cgst = Number(request.cgstAmount || 0);

  const igst = Number(request.igstAmount || 0);

  const gstType = request.gstType || "CGST_SGST";

  const grandTotal = calculatedSubTotal + sgst + cgst + igst;
  ...
  const handleItemChange = (index, field, value) => {
    ...
    const gstAmount = Number(updated[index].gstAmount || 0);

    updated[index].lineTotal = qty * unitPrice + gstAmount;
          ...
          <table className="min-w-full">
            <thead className="bg-slate-100">
              <tr>
                ...
                <th
                  className="px-4 py-4 text-right text-xs md:text-sm font-black uppercase tracking-wider text-slate-700"
                >
                  GST Amount
                </th>
              ...
              {editableItems.map((item, index) => {
                ...
                return (
                  <tr key={index} className="transition hover:bg-slate-50">
                    ...
                    <td className="px-4 py-4 text-slate-700">{item.brand}</td>
